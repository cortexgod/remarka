...
Модель распознавания (`large-v3-turbo`, ~1,6 ГБ) скачивается при первом разборе; для быстрой проверки в настройках можно выбрать `small`.

Слой смысла: в настройках выбрать `claude CLI` (нужен `claude login`) или вставить ключ Anthropic API. Если Claude Code авторизован на твоём сервере, можно указать путь к `scripts/claude-ssh` — тогда модель вызывается по SSH, а с компьютера уходит только текст (см. `scripts/remarka-claude.server`).

Сборка приложения: `npm run tauri build` → `src-tauri/target/release/bundle/macos/Remarka.app` и `.dmg`.
...
- Движок пока не упакован внутрь приложения: нужен `engine/.venv` рядом с проектом или путь к Python в настройках.
