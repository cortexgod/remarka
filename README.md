...
## Слой смысла на своём сервере

Если на VPS уже авторизован Claude Code, модель можно вызывать там, а не логиниться локально:
на сервере лежит `/usr/local/bin/remarka-claude` (запускает `claude` под пользователем с OAuth-токеном),
а в Настройках → «Слой смысла» → «Путь к claude» указывается `scripts/claude-ssh` из этого репозитория.
Обёртка ходит по SSH ключом `~/.ssh/id_ed25519`; хост и ключ переопределяются переменными
`REMARKA_SSH_HOST` и `REMARKA_SSH_KEY`. С компьютера уходит только текст.
