...
Советы, конспект и разбор ответов пишет модель Claude на сервере проекта — туда уходит только текст, без звука. Настраивать ничего не нужно: открыл, ответил на пять вопросов о себе, записал встречу.
...
Сервер советов — `server/remarka-llm.py` (HTTPS с самоподписанным сертификатом, токен, лимит запросов): принимает текст, запускает `claude -p` под авторизованным Claude Code и возвращает JSON. Доступ к нему есть только у автора: адрес, токен и сертификат лежат в `relay.json` в папке данных приложения (`~/Library/Application Support/com.remarka.app/`), в сборку и в репозиторий файл не попадает. В публичной сборке советы, конспект, подготовка и наблюдения между встречами отключены, и приложение говорит об этом прямо; запись, распознавание, показатели, оценка и прогресс работают полностью. Свой сервер поднимается по `server/remarka-llm.service`, после чего достаточно положить свой `relay.json` в папку данных.
...
server/       HTTPS‑ретранслятор советов для своего сервера (systemd‑юнит рядом)
scripts/      SSH‑обёртка (запасной путь для разработчика)
```
