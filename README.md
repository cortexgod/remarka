...
Сервер советов — `server/remarka-llm.py` (HTTPS с самоподписанным сертификатом, токен, лимит запросов): принимает текст, запускает `claude -p` под авторизованным Claude Code и возвращает JSON. Адрес, токен и сертификат кладутся в `engine/remarka_engine/data/relay.json` (не в git) и попадают в сборку. Без этого файла движок использует локальный `claude` или ключ Anthropic, если найдёт, иначе показывает только цифры.
