...
## 13. Изменения после ревью (7 сентября 2026, вторая итерация)

- **§3 CLI**: новая подкоманда `rescore --report R.json --meeting-type T [--baseline B.json] [--calibration-meetings N] [--type-source user|llm|default] [--out …]` — пересчёт готового отчёта под другой тип встречи без ASR (ориентиры, статусы, оценка, сравнение с базой, события `fast_burst`). Оболочка вызывает её из `update_meeting`, когда у готовой встречи меняют тип. `baseline` с пустыми `stats` — ошибка, файл не пишется. `doctor` для `claude_cli` проверяет `claude auth status`: не авторизован → `llm_backend_available=false`.
- **§3.3 ASR**: дорожка транскрибируется по кускам речи по своему VAD (зазоры < 1 с сливаются, куски ≤ 28 с, запас 0,25 с) — промпт дословности действует в каждом окне, тишина не декодируется; `temperature=(0, 0.2, 0.4)` с `compression_ratio_threshold=2.4` против зацикливания. Фильтр галлюцинаций применяется только к подозрительным сегментам (покрытие речью по VAD < 50 %, `no_speech_prob > 0.6`, `avg_logprob < −1`, `compression_ratio > 2.4`) и только по целым типовым фразам.
- **§4**: «тишина» дорожки определяется по 95‑му перцентилю RMS 100‑мс кадров (< −55 дБFS), а не по RMS всей записи; кадры интенсивности ниже −100 дБ (цифровая тишина шумодавов) исключаются из громкости; знаменатель `*_per_min` = Σ mic_speech; граница предложения, выставленная сегментером, — граница мысли. `Cancelled` наследует `BaseException`.
- **§5**: `claude -p … --tools "" --strict-mcp-config --setting-sources ""` — без MCP‑серверов, плагинов и хуков пользователя.
- **§6**: новое событие `recording:warning {meeting_id, message}` (микрофон отключился — запись остановлена и сохранена; системная дорожка оборвалась — микрофон пишется дальше). Тренировка останавливается сама по `duration_sec` задания (`StartParams.max_duration_sec`). Выход из приложения останавливает запущенный движок (SIGTERM → SIGKILL); при старте встречи в статусе `analyzing` с уже записанным `report.json` помечаются `ready`. Системные уведомления (`tauri-plugin-notification`): «Идёт звонок в Zoom — начать запись?» и «Разбор готов / не удался». Zoom считается «в звонке» только по процессу `CptHost` (сам `zoom.us` висит в фоне постоянно); Teams и Телемост — по наличию процесса. Пустое название встречи хранится как NULL. Ключ API не попадает в лог (логируются только программа и аргументы).
- **§7 / выравнивание дорожек**: `ready` от сайдкара ожидается до 190 с (диалог TCC), stderr сайдкара — в `meetings/<id>/tap.log`; момент `ready` фиксируется, и в начало `mic.wav` дописывается тишина на разницу до первого чанка микрофона, так что t=0 у обеих дорожек совпадает. Ошибка сайдкара во время записи попадает в `LevelCell.error` и в `recording:warning`.

## 14. Дизайн, вторая версия (7 сентября 2026)

Редакционный стиль из плана заменён на «инструментальный» в духе Termius: тёмно‑синий фон (`--bg #1f2536`, панель `#1a1f2e`), скруглённые поверхности без рамок (`--bg-2`, радиус 10 px), один синий акцент `#2d8cff`, шрифт Manrope (`@fontsource-variable/manrope`), боковая панель с иконками и «таблеткой» выделения. Убраны: крупные заголовки с подводками, моно‑подписи капсом, теги в рамках, KPI‑плитки (теперь полоса показателей). Заголовок окна — `titleBarStyle: Overlay` (светофоры поверх панели, зоны `data-tauri-drag-region` в панели и над контентом). Светлая тема доступна через настройку `theme: light` (`:root[data-theme="light"]`). Старые имена токенов (`--signal`, `--surface`, `--ink-*`) сохранены как алиасы для SVG‑графиков.
...
## 17. Сервер советов и интерфейс без технических настроек (7 сентября 2026, ночь)

- Бэкенд `remote`: `POST {url}/v1/complete` `{system, user, model, timeout}` с заголовком `X-Remarka-Token` → `{ok, result}`; `GET /v1/health`. Конфиг — `engine/remarka_engine/data/relay.json` `{url, token, cert_pem}` (не в git, в сборке; переопределения `REMARKA_RELAY_URL/TOKEN`). Сертификат самоподписанный и закреплён (проверяется именно он, без проверки имени хоста). Сервер: `server/remarka-llm.py` + `server/remarka-llm.service` (пользователь `assistant`, OAuth из `~/.claude/oauth.env`, до 3 параллельных запросов, 60 запросов в час с одного IP).
- Бэкенд `auto` (по умолчанию в CLI и в оболочке): `remote`, если настроен → `anthropic_api` при ключе → `claude_cli`, если найден → `none`. Оболочка всегда передаёт `--llm auto`; `Settings.llm_backend`, `llm_model`, `llm_cli_path`, `anthropic_api_key`, `asr_model`, `asr_compute_type`, `engine_python` остаются в контракте, но в интерфейсе не показываются.
- Интерфейс: настройки только «Запись», «Внешний вид», «Данные»; из разбора убрана строка про движок и предупреждения ASR; подробные показатели (`MetricDef.advanced`) скрыты за кнопкой «Показать все показатели»; текст о неудаче советов — без названий бэкендов.

## 18. Советы только у автора (8 сентября 2026)

- `relay.json` (адрес, токен, сертификат сервера советов) больше не входит в сборку: `engine/build-sidecar.sh` собирает `data/` без него и падает, если файл всё же оказался в `dist`. Движок ищет файл по `REMARKA_RELAY_FILE` (оболочка передаёт `<папка данных>/relay.json`), затем рядом с собой, затем в `~/Library/Application Support/com.remarka.app/`.
- `AppState.advice_available: boolean` — есть ли доступ к серверу советов. При `false` интерфейс прямо говорит, что советы, конспект, подготовка и наблюдения между встречами в этой сборке недоступны; движок при `--llm auto` без сервера пишет предупреждение «Слой смысла отключён: сервер советов не настроен» и строит отчёт без слоя смысла.
- Токен сервера сменён; DMG версий 0.3.0 и 0.4.0 содержали старый токен и сняты с релизов, вместо них — 0.4.1.
